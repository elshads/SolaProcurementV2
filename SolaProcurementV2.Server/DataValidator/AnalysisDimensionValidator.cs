...
{
    public class AnalysisDimensionValidator : AbstractValidator<AnalysisDimension>
    {
        public AnalysisDimensionValidator()
        {
        ...
        private bool UniqueCode(string code)
        {
            if (!string.IsNullOrEmpty(code))
            {
                try
                {
                    ...
                    {
                        var sql = $"SELECT Code FROM AnalysisDimension";
